...
def test_happy_path():
    reader = PlyReader("test_data/common/test_happy_path.ply")
    reader.run()

    assert reader.filename == "test_data/common/test_happy_path.ply"
...
def test_zero_at_end():
    reader = PlyReader("test_data/basic_tests/test_zero_at_end.ply")
    reader.run()
...
def test_zero_at_start():
    reader = PlyReader("test_data/basic_tests/test_zero_at_start.ply")
    reader.run()
...
def test_no_extra_lines_at_end():
    reader = PlyReader("test_data/basic_tests/test_no_extra_lines.ply")
    reader.run()
...
def test_empty_output():
    reader = PlyReader("test_data/basic_tests/test_empty_output.ply")
    reader.run()
...
def test_no_header_size():
    filename = "test_data/basic_tests/test_no_end_header.ply"
    reader = PlyReader(filename)
...
def test_no_entries_count():
    filename = "test_data/basic_tests/test_no_size.ply"
    reader = PlyReader(filename)
    with pytest.raises(Exception) as e:
        reader.run()
